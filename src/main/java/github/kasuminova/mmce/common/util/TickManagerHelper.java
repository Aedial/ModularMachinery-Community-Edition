...
public final class TickManagerHelper {
    ...
    public static boolean reRegisterTickable(IGridNode node, IGridTickable tickable) {
        ...
        if (!(tickable instanceof IGridHost)) {
            ModularMachinery.log.warn("Tickable is not an IGridHost: {}", tickable);
            return false;
        }
    ...
    private static void purgeStaleTrackers(ITickManager tickManager, IGridNode node) {
        if (reflectionFailed || !(tickManager instanceof TickManagerCache)) {
            return;
        }
        ...
        try {
            ...
            ModularMachinery.log.warn(
                "Failed to purge stale AE2 tick trackers. Polling rate changes may tick incorrectly until reload.",
                e
            );
        }
    }
